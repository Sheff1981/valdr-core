...
**Stage 8 — Bitcoin/Litecoin-style bootstrap + P2P protection: implemented and CI-verified (software gate), with v0.2.6 peer-cache/DNS-seed extension pending CI.**

V2 nodes bootstrap from a persistent learned-peer cache, optional compiled fixed seeds, operator `--seed host:port` overrides, optional DNS seeds, and then peer exchange. Bootstrap failures are best-effort. A background maintainer retries bootstrap/discovered peers toward the outbound target of 8. No seed is a consensus authority.
...
**Distributed Testnet bootstrap note:** the repository does not invent public seed addresses. VALDR does not require project-owned or rented public servers. Real reachable endpoints may later come from independently operated user full nodes, volunteer fixed seeds or DNS-seed operators. Until such endpoints exist, users can connect through learned peer cache or explicit `--peer`/`--seed` addresses. A network cannot cold-start from zero reachable peers; bootstrap contacts are discovery helpers, never consensus authorities.
...
Actual Testnet bootstrap endpoints are deployment data, not fabricated placeholders. They may be supplied by independently operated user nodes, volunteer fixed seeds or DNS-seed operators. Project-owned paid servers are not a release prerequisite. `NetworkTestnetV02.DefaultSeeds`/`DNSSeeds` stay empty until real independently operated endpoints exist.
