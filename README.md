...
**Stage 12 — VALDR Desktop: in progress.** The current slice includes the Wails v2 shell, Testnet-only first-run encrypted-wallet setup, managed outbound-only local node lifecycle, sync progress, balance/send/receive, reorg-safe transaction history, encrypted backup/restore, explicit surfacing/restart recovery for an unexpectedly exited managed node, and local wallet lock/unlock with configurable inactivity auto-lock. Stage 12 is not complete until the full Desktop acceptance gate in Master-TZ v0.2.3 is green across the mandatory OS matrix. Private/Public Testnet remains behind completed Stages 12 and 13.
