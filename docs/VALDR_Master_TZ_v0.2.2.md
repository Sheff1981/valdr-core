# VALDR / MASTER TECHNICAL SPECIFICATION v0.2.2

**Расширение Testnet, усиление протокола и подготовка публичных нод**  
**Дата:** 23 сентября 2026  
**Статус:** рабочий мастер-ТЗ / implementation baseline; sync-wire amendment v0.2.2  
**Baseline:** VALDR Devnet v0.1 commit `3f322cf8f519cfe8dc350ffc83081c8b82bcd4be`

> v0.2.2 не переписывает завершённый v0.1. Frozen reference v0.1: `release/valdr-devnet-v0.1`. v0.2 и v0.2.1 остаются сохранёнными предыдущими редакциями; v0.2.2 фиксирует JSON payload-контракт headers-first sync поверх уже замороженного header v2.
...
### 13.1 P2P v2 sync payload contract

All payloads use the strict UTF-8 JSON rules from §12. Unknown fields are rejected.

Hash strings are exactly 64 lowercase hexadecimal characters.

`get_headers`:

```json
{"locator":["<hash>", "..."],"stop_hash":""}
```

- `locator`: newest-to-oldest block hashes, 1..128 entries;
- locator generation: recent consecutive hashes, then exponential backoff, always ending at Genesis;
- `stop_hash`: empty string means no explicit stop; otherwise exact block hash;
- receiver selects the first locator hash present on its active chain.
...
- 0..2,000 headers;
- each header uses the network's frozen header version and canonical hash rules;
- every non-empty batch must be internally continuous;
- receiver validates chain-id/version/linkage/target/timestamp/PoW before requesting any body.

`get_data`:

```json
{"items":[{"kind":"block","hash":"<hash>"}]}
```

- 1..32 items per sync request;
- `kind` is `block` for Stage 7 body synchronization;
- duplicate hashes in one request are invalid.
...
- its header/hash must equal a previously validated/requested header before normal block/body validation.

`get_blocks` uses the same `locator` + `stop_hash` payload as `get_headers` and returns `inv`.
...
- 0..32 block inventory items for Stage 7 synchronization;
- order is active-chain order after the common ancestor.
...
and is admitted through the Stage 6 mempool policy.

A node may restart a sync session by sending a fresh locator. Sync-session state itself is not consensus state and need not be persisted; validated blocks/side branches are persisted, so reconnect/restart resumes from stored chain data without deleting the DB.
...
- Master specs v0.2 and v0.2.1 remain preserved; v0.2.2 is the active implementation baseline from the commit that adds this file.
...
Wire-contract clarification required for Stage 7:
1. freeze strict JSON payload shapes for `get_headers/headers/get_data/block/get_blocks/inv/tx`;
2. bound locator to 128 hashes, headers to 2,000, and outstanding/requested block bodies to 32;
3. require a received block body to match a previously validated/requested header;
4. define restart behavior: session state may reset, persisted blocks/branches remain the resume source.

Benefits: interoperable headers-first sync, bounded memory/network work, deterministic resume behavior, and no body download before header validation.

Risks: peers implementing older experimental v2 sync payloads are wire-incompatible and must be upgraded together. This does not affect frozen v0.1 P2P v1.
