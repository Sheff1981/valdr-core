# VALDR / MASTER TECHNICAL SPECIFICATION v0.2.3

**Desktop, cross-platform distribution and Testnet productization**  
**Date:** 24 September 2026  
**Status:** active master implementation baseline  
**Previous baseline:** `docs/VALDR_Master_TZ_v0.2.2.md`  
**Active branch:** `valdr-v0.2`

> v0.2.3 replaces v0.2.2 only as the current planning/implementation baseline. v0.2, v0.2.1 and v0.2.2 remain immutable historical specifications. Frozen legacy v0.1 remains `release/valdr-devnet-v0.1`.
...
| 12 | VALDR Desktop | planned / not started |
| 13 | Cross-platform installers + release pipeline | planned / not started |
| 14 | Private/Public Testnet | planned / not started |
...
- Docker/systemd for public infrastructure.
...
## 10. Transactions, fees and mempool
...
- connect outbound to seeds/discovered peers;
...
## 24. Stage 14 — Private/Public Testnet

Only after Stages 12 and 13 are green.

Infrastructure requirements:

- >=3 stable public full nodes;
- >=2 independent regions/providers;
- real routable seed endpoints compiled into Testnet profile;
- optional independent observer.

Sequence:

1. private soak >=24 h;
2. fix all consensus/storage/sync/wallet/desktop blockers;
3. freeze Testnet release candidate;
4. publish signed Desktop installers/packages;
5. public Testnet announcement;
6. >=7 days public operation without consensus split or manual DB repair.
...
- v0.2.3 is the active master specification after its commit;
...
- at least 3 real public Testnet nodes are available;
- public Testnet remains stable for >=7 days without consensus split/manual DB repair;
- Mainnet remains disabled.

## 30. v0.2.3 change log

Changes relative to v0.2.2:

1. Preserve all implemented Stage 0-11 consensus/storage/P2P/wallet/Explorer/Testnet behavior.
2. Remove obsolete planning language that said Testnet Genesis was not frozen; it is now frozen.
3. Add VALDR Desktop as mandatory product stage before public Testnet.
4. Add cross-platform Windows/macOS/Linux packaging and release-signing stage.
5. Renumber former private/public Testnet stage from 12 to 14 and Mainnet draft from 13 to 15.
6. Add mandatory official reference research using Bitcoin Core, Monero GUI, Litecoin Core and Ethereum official resources before major Desktop/download UX decisions.
7. Define Wails v2 stable as the initial Desktop framework while keeping consensus code in Go.
8. Define local wallet signing + managed local node process; Desktop must not duplicate consensus.
9. Define Simple/Advanced modes and minimum wallet/node screens.
10. Define default outbound-only Desktop networking so ordinary users need not open inbound P2P ports.
11. Define Windows/macOS/Linux artifact matrix, code signing/notarization, checksums and signed release manifest.
12. Define official download-page requirements and no automatic unsigned updates.
13. Move public Testnet behind completed Desktop/installers gates.
...
No Mainnet launch is authorized by v0.2.3.
