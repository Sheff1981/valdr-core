...
Manual re-verification accepts only a completed successful `VALDR v0.2 CI` run on `valdr-v0.2`. Automatic execution is a same-commit reusable-workflow call from the CI run itself, so the source run is necessarily still `in_progress`; that path verifies the exact run ID, commit, branch and workflow while the caller `needs` requires `build-test-storage` plus the clean Stage 13 verification chain. Manual dispatch still requires the source run ID and commit explicitly and requires the source run to be completed successfully. The earlier `workflow_run` approach was rejected because this development workflow lives on `valdr-v0.2` while GitHub only recognizes `workflow_run` triggers from workflow files present on the default branch.
...
The workflow has no release/tag/write permission and cannot publish a GitHub Release. The caller depends on `build-test-storage` and the clean verification job; the latter transitively depends on all Desktop packaging and canonical assembly jobs. Therefore a failed/cancelled required source-CI job cannot reach the same-run handoff. This preserves the v0.2.8 rule that Stage 12 manual Windows acceptance remains a public-release blocker while proving the exact source-run → commit → artifacts handoff boundary.
