...
| Exact CI-run → release handoff | `valdr-v02-release.yml` is called as a same-commit reusable workflow after clean release verification; CI passes its own run ID + exact commit, the handoff re-verifies manifest/checksums/provenance and emits a non-public record; manual re-verification remains available; no release publication permission | implemented; execution evidence pending next green CI |
