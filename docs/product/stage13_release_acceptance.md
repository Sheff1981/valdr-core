...
| Exact CI-run → release handoff | `valdr-v02-release.yml` automatically follows successful `VALDR v0.2 CI` runs on `valdr-v0.2`, derives exact source run/commit from `workflow_run`, re-verifies manifest/checksums/provenance and emits a non-public handoff record; manual re-verification remains available; no release publication permission | implemented; automatic execution evidence pending next green CI |
