# VALDR / CONSOLIDATED MASTER TECHNICAL SPECIFICATION v0.2.13

**Implementation-first engineering baseline + deferred end-to-end validation**  
**Date:** 26 September 2026  
**Status:** active master implementation baseline  
**Previous baseline:** `docs/VALDR_Master_TZ_v0.2.12.md`  
**Active branch:** `valdr-v0.2`

> v0.2.13 replaces v0.2.12 as the single working master specification. Historical specifications v0.2 through v0.2.12 remain immutable evidence of prior decisions. Frozen legacy v0.1 remains `release/valdr-devnet-v0.1`.

> This revision is intentionally cumulative and implementation-first. It preserves all frozen Testnet2 consensus/network values and the existing architecture, but changes execution order: engineering implementation and automated CI hardening are completed first; manual, multi-machine and soak-style acceptance are deferred to one final end-to-end validation phase. The project must work from this file first instead of reconstructing the plan from older documents.
...
10. Historical stage numbers are requirement identifiers, not permission barriers. Where this v0.2.13 explicitly defers an acceptance gate, technical work from later hardening sections may be pulled forward without claiming the deferred stage complete.
...
## 4. Current repository reality inherited into v0.2.12
...
## Stage 12C — Manual Windows acceptance (deferred final validation)
...
**Status:** deferred final gate; do not block technical implementation.

Purpose: validate the completed technical milestone on real independent computers/clients after Core, P2P, storage, RPC, wallet, Desktop and packaging hardening are complete.
...
1. **R0 complete:** green Testnet2 baseline is commit `20e725fb52673a25945267a544a09a14ab195033` (CI #440, success).
...
15. Run Stage 14A final independent multi-machine validation: at least three separate sessions, approximately 2-3 hours each.
16. Stage 14B public/volunteer Testnet validation is an optional extension and is not a v0.2 release-blocking duration gate.
17. Collect Stage 14C longer-term evidence optionally; it is not a v0.2 blocking gate.
